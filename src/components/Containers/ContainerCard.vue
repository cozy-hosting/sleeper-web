<template>
  ...
  >
    ...
    <a-descriptions layout="vertical" :column="1">
      ...
      <a-descriptions-item label="Ports" v-if="notEmpty(container.ports)">
        {{ container.ports | joinComma | joinPort }}
      </a-descriptions-item>
      <a-descriptions-item
        label="Addresses"
        v-if="notEmpty(container.addresses)"
      >
        {{ container.addresses | joinComma }}
      </a-descriptions-item>
      <a-descriptions-item label="Labels" v-if="notEmpty(container.labels)">
        <div
          style="width: 100%"
...
export default class ContainerCard extends Vue {
  ...
  notEmpty(x: Record<string, any> | any[]) {
