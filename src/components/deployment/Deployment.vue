<template>
    <div class="deployment">
            <a-alert v-if='alertMessage.length > 0'
      message="Error"
      type="error"
      closable
      @close="onCloseAlert"
    ><div slot="description" v-for="alert in alertMessage" :key="alert">{{alert}}</div></a-alert>
    ...
    <a-col :span="24">
                                <a-divider></a-divider>
    <a-list v-if="allDeployments.length > 0"  size="small" item-layout="horizontal" :data-source="allDeployments">
                ...
                <a-list-item slot="renderItem" slot-scope="item, index">
                    <a-avatar >{{index}}</a-avatar>
                        ...
                        <a slot="actions" v-on:click="del(item)">delete</a>
                                            <a-list-item-meta
                        ><div slot="description">{{item.description}}</div></a-list-item-meta>
                ...
                <a-button type="primary" @click="showDrawer" style="margin-top: 15px"> <a-icon type="plus" /> Deploy </a-button>
    <a-drawer
    ...
    >
        <a-form-model :model="form" layout="vertical" hide-required-mark>
            <a-form-model-item label="Deployment name">
                <a-input v-model="form.name" placeholder="Name for your new Deployment" />
            </a-form-model-item>

            <a-form-model-item label="Description">
                <a-textarea v-model="form.description" placeholder="Describe your new Deployment" :auto-size="{ minRows: 3, maxRows: 6 }" />
            </a-form-model-item>
        ...
        <a-form-model-item>
            <a-row :gutter="16">
            <a-col :span="12">
            <a-list size="small" item-layout="horizontal" :data-source="currentImages">
                <a-list-item   slot="renderItem" slot-scope="item, index" v-if="index < 3">
            ...
            <a-col :span="12" style="margin-top: 35px">
                ...
                <a-input name="imgLabel" placeholder="Search your Image or paste Image URL" v-on:change="imgChange()" v-model="form.image"></a-input>
            </a-col>
            </a-row>
        </a-form-model-item>
        <a-form-model-item>
                        <a-divider>Mounts</a-divider>
                        <a-list size="small" item-layout="horizontal" :data-source="form.mounts">
                        <a-list-item slot="renderItem" slot-scope="item, index">
                        <a-input v-model="form.mounts[index]" placeholder="Enter your data mounts">  </a-input>
                        </a-list-item> 
                        </a-list>
                        <a-button type="primary" @click="addMount"> <a-icon type="plus" /> Add </a-button>
        </a-form-model-item>
                <a-form-model-item>
                        <a-divider>Ports</a-divider>
                        <a-list size="small" item-layout="horizontal" :data-source="form.ports">
                        <a-list-item slot="renderItem" slot-scope="item, index">
                        <a-input v-model="form.ports[index]" placeholder="Pass your port (PORT-TO:PORT-FROM)">  </a-input>
                        </a-list-item> 
                        </a-list>
                        <a-button type="primary" @click="addPort"> <a-icon type="plus" /> Add </a-button>
                </a-form-model-item>

                <a-form-model-item>
                        <a-divider>Environment</a-divider>
                        <a-list size="small" item-layout="horizontal" :data-source="form.environment">
                        <a-list-item slot="renderItem" slot-scope="item, index">
                        <a-input type="text" v-model="form.environment[index].k" style="margin-right: 10px" placeholder="Key">  </a-input>
                        <a-input type="text" v-model="form.environment[index].val" style="margin-left: 10px" placeholder="Value">  </a-input>
                        </a-list-item>
                        
                        </a-list>
                        <a-button type="primary" @click="addEnv" v-if="form.labels.length <=3"> <a-icon type="plus" /> Add </a-button>
                </a-form-model-item>

                <a-form-model-item>
                        <a-divider>Links</a-divider>
                        <a-list size="small" item-layout="horizontal" :data-source="form.links">
                        <a-list-item slot="renderItem" slot-scope="item, index">
                        <a-input type="text" v-model="form.links[index].k" style="margin-right: 10px" placeholder="Key">  </a-input>
                        <a-input type="text" v-model="form.links[index].val" style="margin-left: 10px" placeholder="Value">  </a-input>
                        </a-list-item>
                        
                        </a-list>
                        <a-button type="primary" @click="addLink" v-if="form.labels.length <=3"> <a-icon type="plus" /> Add </a-button>
                </a-form-model-item>


                <a-form-model-item>
                        <a-divider>Labels</a-divider>
                        <a-list size="small" item-layout="horizontal" :data-source="form.labels">
                        <a-list-item slot="renderItem" slot-scope="item, index">
                        <a-input type="text" v-model="form.labels[index].k" style="margin-right: 10px" placeholder="Key">  </a-input>
                        <a-input type="text" v-model="form.labels[index].val" style="margin-left: 10px" placeholder="Value">  </a-input>
                        </a-list-item>
                        
                        </a-list>
                        <a-button type="primary" @click="addLabel" v-if="form.labels.length <=3"> <a-icon type="plus" /> Add </a-button>
                </a-form-model-item>
  ...
  <a-timeline mode="right" style="margin-right: 50%; margin-top: 25px" v-if="deployState != 0">
    <a-timeline-item>
        <a-icon slot="dot" type="clock-circle-o" style="font-size: 16px;" v-if="deployState == 1" />
        Create your Deployment</a-timeline-item>
    <a-timeline-item>
        <a-icon slot="dot" type="clock-circle-o" style="font-size: 16px;" v-if="deployState == 2" />
        Pull your image from the repository</a-timeline-item>
    <a-timeline-item>
        <a-icon slot="dot" type="clock-circle-o" style="font-size: 16px;" v-if="deployState == 3" />
...
export default class Deployment extends Vue {
    ...
    async getAllDeployments(skip: number, take: number)
    {
        ...
        this.allDeployments = (await this.deployService.getAll(skip, take)).data.data;
    }
    ...
    validate()
    {
        ...
        {
            this.alertMessage.push("You haven't entered a description for the deployment.");
        }
        if(!this.form.image)
        {
