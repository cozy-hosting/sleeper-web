...
export default class Deployment extends Vue {
  ...
  created() {
    this.getAllDeployments(0, 20);
  }

  async getAllDeployments(skip: number, take: number) {
    const res = await DeploymentService.getAll(skip, take);
    this.allDeployments = res.data;
  }

  async showDrawer() {
    this.visible = true;
    this.currentImages = (await ImageService.getAll(0, 20)).data.data;
  ...
  async onSubmit() {
    ...
    try {
      ...
      const deploy = await DeploymentService.create(this.form);

      this.deployState = DeployState.PULL;
      const pull = await ImageService.create({ deployment: deploy.data.data });

      this.deployState = DeployState.CCONTAINER;
      const cont = await ContainerService.create(deploy.data.data);

      this.deployState = DeployState.RCONTAINER;
      const contStart = await ContainerService.start(cont.data.data);
  ...
  async handleOk() {
    ...
    const edit = await DeploymentService.update(json.id, json);
    this.editMode = false;
  ...
  async handleDelete() {
    if (this.deleteContainer) {
      const all: any[] = (await ContainerService.getAll(0, 50)).data.data;
      const el = all.find((x: any) => x.deployment == this.selectedItem.id);
      await ContainerService.delete(el.id);
    }

    await DeploymentService.delete(this.selectedItem.id);
    this.getAllDeployments(this.skip, this.take);
